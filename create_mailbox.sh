...
MYSQL_DB="vmail"
MYSQL_USER="root"
MYSQL_PASS="4hFBgy2QSW3Sv51RbwMIjG033Ny4qa08"  # ❗ REPLACE or read securely from env or prompt

# Validate input
if [ $# -ne 2 ]; then
    echo "Usage: $0 <email> <plain_password>"
    exit 1
fi
...
HASHED_PASS=$(doveadm pw -s "$PASSWORD_SCHEME" -p "$PLAIN_PASS")
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to insert user $EMAIL into database."
    exit 1
fi
...
echo "📂 Maildir path: $MAILDIR_PATH"
